...
| Read-shaped payloads use `Match` (or `Project`/`Aggregate`/`Subscribe`); write-shaped payloads use `Assert`/`Mutate`/`Retract`/`Atomic`. | The per-variant `sema_verb()` mapping enforces this; introspect is read-only inspection plane so all current variants are `Match`. |
